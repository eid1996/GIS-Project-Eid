...
def load_data(uploaded_file):
    if uploaded_file:
        ...
        try:
            # إذا كان الملف مضغوط ZIP (Shapefile)
            if file_extension == ".zip":
                gdf = gpd.read_file(f"zip://{path}")
            else:
                # للملفات الأخرى مثل GeoJSON
...
with col1:
    ...
    f1 = st.file_uploader("ارفع ملف (Shapefile ZIP أو GeoJSON)", key="u1")
    gdf1 = load_data(f1)
    if gdf1 is not None: 
        st.success("تم تحميل الطبقة الأولى")
        st.write("أول 5 صفوف من البيانات:") # إضافة طلب المهندس
        st.dataframe(gdf1.head()) # عرض الجدول
...
with col2:
    ...
    f2 = st.file_uploader("ارفع ملف (Shapefile ZIP أو GeoJSON)", key="u2")
    gdf2 = load_data(f2)
    if gdf2 is not None: 
        st.success("تم تحميل الطبقة الثانية")
        st.write("أول 5 صفوف من البيانات:") # إضافة طلب المهندس
        st.dataframe(gdf2.head()) # عرض الجدول
